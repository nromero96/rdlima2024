$(document).ready(function() {
    ...
    function favNote() {
        $(".fav-note").off('click').on('click', function(event) {
          ...
          var favourites = $(this).parents('.note-item').hasClass('note-fav') == true ? 'note-fav' : '';
          var note_id = $(this).data('noteid');

          $.ajax({
              type: "GET",
              dataType: "json",
              url: baseurl+"/changefavourite-note",
              data: {'favourites': favourites, 'note_id': note_id},
              success: function(data){
                console.log(data.success)
              },
              error: function(data){
    ...
    function addLabelGroups() {
        $('.tags-selector .label-group-item').off('click').on('click', function(event) {
          ...
          var tag = getSplitclass;
          var note_id = $(this).data('noteid');

          $.ajax({
            type: "GET",
            dataType: "json",
            url: baseurl+"/changetag-note",
                  ...
                  '<div class="note-footer">' +
                      '<div class="tags-selector btn-group">' +
                          ...
                          '<div class="dropdown-menu dropdown-menu-right d-icon-menu">' +
                              '<a class="note-personal label-group-item label-personal dropdown-item position-relative g-dot-personal" href="javascript:void(0);" data-noteid="'+data.noteid+'"> Personal</a>' +
                              '<a class="note-work label-group-item label-work dropdown-item position-relative g-dot-work" href="javascript:void(0);" data-noteid="'+data.noteid+'"> Work</a>' +
                              '<a class="note-social label-group-item label-social dropdown-item position-relative g-dot-social" href="javascript:void(0);" data-noteid="'+data.noteid+'"> Social</a>' +
                              '<a class="note-important label-group-item label-important dropdown-item position-relative g-dot-important" href="javascript:void(0);" data-noteid="'+data.noteid+'"> Important</a>' +
                          '</div>' +
                      '</div>' +
                  '</div>' +
              '</div>' +
